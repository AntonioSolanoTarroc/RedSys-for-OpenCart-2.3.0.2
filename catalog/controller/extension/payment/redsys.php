...
class ControllerExtensionPaymentRedsys extends Controller {
	public function index() {
		...
		$data['Clave256']=$this->config->get('redsys_clave256');
		...
		$data['LogActivo']=$this->config->get('redsys_log');
		$data['MantenerPedidoAnteError']=$this->config->get('redsys_error_pedido');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/redsys.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/extension/payment/redsys.tpl', $data);
		} else {
			return $this->load->view('extension/payment/redsys.tpl', $data);
		}		
	...
	public function callback() {
		...
		$logActivo	= $this->config->get('redsys_log');
		$mantenerPedidoAnteError	= $this->config->get('redsys_error_pedido');

			if (isset($this->request->get['Ds_MerchantParameters'])) {
								
				// Recogemos la clave del comercio para autenticar
				$clave256     = $this->config->get('redsys_clave256');	
				...
				// Validacion de firma y parámetros
						if ($firma_local === $firma_remota
							...
							if ($respuesta < 101){
									//$this->model_checkout_order->addOrderHistory($pedido, 5);
									$this->escribirLog("Pago aceptado. Para el pedido ".$idPedido,$logActivo);
									$this->response->redirect($this->url->link('checkout/success'));
							} else {	
									//$this->model_checkout_order->addOrderHistory($pedido, 7);
			...
			else if (isset($this->request->post['Ds_MerchantParameters'])){
